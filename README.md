...
Animation controls include the global `animationSpeed` multiplier (scales word reveals + camera tracking), `lineStartDelayMs` (adds a short pre-roll when `keepPreviousLinesVisible` is off so the camera can settle before words begin), and punctuation pacing (`commaPauseMs`, `sentencePauseMs`, `dashPauseMs`) to add natural pauses between commas and sentence endings in fallback/cue timing.
